...
export default class AltanProductTypeModel {
    ...
    public async validate(): Promise<any> {
        return this.validateSchemas(this.productType);
    }

    private async validateSchemas(_productType: any = {}): Promise<any> {
        
        let errors: Array<any> = [];
        
        let productType = new ProductType();
        productType.name = _productType.name;
        productType.value = _productType.value;
        let productTypeErrors = await validate(productType, { skipMissingProperties: true });
        errors = RemodelErrors(productTypeErrors, 'productType');
    ...
    public getProductType(query: any): Promise<any> {
        return new Promise((resolve: (value?: {} | PromiseLike<{}> | undefined) => void, reject: (reason?: any) => void) => {
            axios.get(configuration.services.altan.products.types.getType, { params: query })
            .then((response: any) => {
    ...
    public getProductTypes(query: any): Promise<any> {
        // Parámetros.
        ...
        // Petición.
        return new Promise((resolve: (value?: {} | PromiseLike<{}> | undefined) => void, reject: (reason?: any) => void) => {
            axios.get(configuration.services.altan.products.types.getTypes, { params })
            .then((response: any) => {
                return resolve(idx(response, _ => _.data) || []);
            })
            .catch((error: any) => {
    ...
    public postProductType(body: any): Promise<any> {
        return new Promise((resolve: (value?: {} | PromiseLike<{}> | undefined) => void, reject: (reason?: any) => void) => {
            let errors: Array<any> = [];
            ...
            if(errors.length > 0) {
                ...
                axios.post(configuration.services.altan.products.types.postType, body)
                .then((response: any) => {
                    return resolve(idx(response, _ => _.data) || {});
                })
                .catch((error: any) => {
                    if(typeof error.code === 'string' && error.code === 'ECONNREFUSED') {
                        ...
                        return reject({
                            status: 400,
                            message: 'Ocurrió un error al intentar guardar la información (PRODUCTO - TIPO).',
                            error: idx(error, _ => _.response.data) || error
                        });
                    }
                });
    ...
    public putProductType(body: any): Promise<any> {
        return new Promise((resolve: (value?: {} | PromiseLike<{}> | undefined) => void, reject: (reason?: any) => void) => {
            let errors: Array<any> = [];
            this.validateSchemas(body)
            .then((data: any) => {
            ...
            if(errors.length > 0) {
                ...
                axios.put(configuration.services.altan.products.types.putType, body)
                .then((response: any) => {
                    return resolve(idx(response, _ => _.data) || {});
                })
                .catch((error: any) => {
    ...
    public deleteProductType(query: any): Promise<any> {
        return new Promise((resolve: (value?: {} | PromiseLike<{}> | undefined) => void, reject: (reason?: any) => void) => {
            axios.delete(configuration.services.altan.products.types.deleteType, { params: query })
            .then((response: any) => {
                return resolve(idx(response, _ => _.data) || {});
            })
            .catch((error: any) => {
