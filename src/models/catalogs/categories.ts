...
export default class CategoryModel {
    ...
    public async validate(): Promise<any> {
        return this.validateSchemas(this.category);
    }

    private async validateSchemas(_category: any = {}): Promise<any> {
        ...
        let categoryErrors = await validate(category, { skipMissingProperties: true });
        errors = RemodelErrors(categoryErrors, 'category');
    ...
    public getCategory(query: any): Promise<any> {
        return new Promise((resolve: (value?: {} | PromiseLike<{}> | undefined) => void, reject: (reason?: any) => void) => {
            axios.get(configuration.services.domain.products.categories.getCategory, { params: query })
            .then((response: any) => {
    ...
    public getCategories(query: any): Promise<any> {
        // Parámetros.
        ...
        // Petición.
        return new Promise((resolve: (value?: {} | PromiseLike<{}> | undefined) => void, reject: (reason?: any) => void) => {
            axios.get(configuration.services.domain.products.categories.getCategories, { params })
            .then((response: any) => {
                return resolve(idx(response, _ => _.data) || []);
            })
            .catch((error: any) => {
    ...
    public postCategory(body: any): Promise<any> {
        return new Promise((resolve: (value?: {} | PromiseLike<{}> | undefined) => void, reject: (reason?: any) => void) => {
            let errors: Array<any> = [];
            ...
            if(errors.length > 0) {
                ...
                axios.post(configuration.services.domain.products.categories.postCategory, body)
                .then((response: any) => {
                    return resolve(idx(response, _ => _.data) || {});
                })
                .catch((error: any) => {
                    if(typeof error.code === 'string' && error.code === 'ECONNREFUSED') {
                        ...
                        return reject({
                            status: 400,
                            module: 'Categorías',
                            message: 'Ocurrió un error al intentar guardar la información (CATEGORÍA).',
                            error: idx(error, _ => _.response.data) || error
                        });
                    }
                });
    ...
    public putCategory(body: any): Promise<any> {
        return new Promise((resolve: (value?: {} | PromiseLike<{}> | undefined) => void, reject: (reason?: any) => void) => {
            let errors: Array<any> = [];
            this.validateSchemas(body)
            .then((data: any) => {
            ...
            if(errors.length > 0) {
                ...
                axios.put(configuration.services.domain.products.categories.putCategory, body)
                .then((response: any) => {
                    return resolve(idx(response, _ => _.data) || {});
                })
                .catch((error: any) => {
    ...
    public deleteCategory(query: any): Promise<any> {
        return new Promise((resolve: (value?: {} | PromiseLike<{}> | undefined) => void, reject: (reason?: any) => void) => {
            axios.delete(configuration.services.domain.products.categories.deleteCategory, { params: query })
            .then((response: any) => {
                return resolve(idx(response, _ => _.data) || {});
            })
            .catch((error: any) => {
